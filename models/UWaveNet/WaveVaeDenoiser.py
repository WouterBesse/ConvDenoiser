...
import torchaudio
import models.UWaveNet.WaveVaeOperations as WOP
import models.UWaveNet.WaveVaeWavenet as WaveNet
...
class UNet(nn.Module):
    ...
    def __init__(self, input_size, hidden_dim = 768, zsize = 128, resblocks = 2, relublocks = 4):
        ...
        features, timesteps = input_size
        self.contract1 = WOP.contract_block(1, 64, 3, 1)
        self.contract2 = WOP.contract_block(64, 128, 3, 1)
        self.contract3 = WOP.contract_block(128, 256, 3, 1)
        self.contract4 = WOP.contract_block(256, 512, 3, 1)
        
        self.expand4 = WOP.expand_block(512, 256, 3, 1)
        self.expand3 = WOP.expand_block(256*2, 128, 3, 1)
        self.expand2 = WOP.expand_block(128*2, 64, 3, 1)
        self.expand1 = WOP.expand_block(64*2, 1, 3, 1)
    ...
    def forward(self, x):
        ...
        # print(conv3.size())
        conv4 = self.contract4(conv3)
        # print(conv4.size())
        
        x = self.expand4(conv4)
        # print(x.size())
        x = self.expand3(torch.cat([x, conv3], 1))
        x = self.expand2(torch.cat([x, conv2], 1))
        x = self.expand1(torch.cat([x, conv1], 1))
        return x
    

class WaveNetUnet(nn.Module):

    def __init__(self, input_size, device, num_hiddens, dil_rates, zsize = 128, resblocks = 2, out_channels = 256):
        super(WaveNetUnet, self).__init__()
    ...
    def forward(self, xau, xspec, jitter):
        ...
        """
        xspec = self.encoder(xspec)
        # print(xspec.size())
        # z = self.sample(mean, var)

        x_hat = self.decoder(xau, xspec, jitter)
...
class WaveVaeDataset(Dataset):
    ...
    def __init__(self, clean_folder, noisy_folder, clip_length = 512, clips = 1, one_hot=True):
        ...
        mfcc_trans = MFCC(32000, 64, log_mels = True, melkwargs={"hop_length": 32}) # Create MFCC in the right samplerate
            ...
            for f in clean_filepaths:
                ...
                if os.path.isfile(os.path.join(clean_folder, f)):
                   ...
                   # print(clean_audiopath, noisy_audiopath)
                    ...
                    i = 512 # Start 256 samples in so it has information about the past

                    while i < clean_audiofile.size()[-1] - 2048:
